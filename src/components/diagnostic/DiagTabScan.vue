...
async function exportScanTxt() {
  ...
  try {
    ...
    useNotificationStore().success("Scan exporté", filePath);
  } catch (e: any) {
    const { useNotificationStore } = await import("@/stores/notifications");
    useNotificationStore().error("Erreur export", String(e));
  }
}
