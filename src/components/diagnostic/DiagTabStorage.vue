...
<template>
  ...
  <template v-if="tab === 'disks'">
    ...
    <template v-else>
      <p class="diag-section-label">Disques physiques — {{ storageList.length }} détecté(s)</p>
      <div v-for="(d, i) in storageList" :key="i" class="card-block">
        ...
        <template v-if="smartData.length > 0">
          <p class="diag-section-label" style="margin-top:12px;margin-bottom:8px">
            Santé SMART
            <span v-if="!smartForDisk(d.model)" class="muted" style="font-weight:400;font-size:11px"> — données non associées</span>
          </p>
          <template v-if="smartForDisk(d.model) as smart">
            <div class="info-grid">
              <!-- Santé globale -->
              <div class="info-row">
